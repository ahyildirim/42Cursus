...
{
	pthread_mutex_lock(&philo->table->forks[philo->lhand]);
	philo_msg("has taken a fork", philo->table, philo->id);
	if(philo->table->number_of_philo == 1)
		return (1);
	pthread_mutex_lock(&philo->table->forks[philo->rhand]);
	philo_msg("has taken a fork", philo->table, philo->id);
	...
	philo->meals++;
	pthread_mutex_unlock(&philo->table->forks[philo->rhand]);
	pthread_mutex_unlock(&philo->table->forks[philo->lhand]);
	return (0);
}
...
void check_if_dead(t_table *table)
{
	while(table->is_full)
	{
		table->i = -1;
		while(++table->i < table->number_of_philo && table->is_died)
		{
			pthread_mutex_lock(&table->die_check);
			if(time_diff(table->philo[table->i].last_eat_time, get_current_time()) > table->time_to_die)
			{
		...
		if(table->is_died == 0)
			break ;
		table->i = 0;
		while(table->i < table->number_of_philo && table->max_meals != -1 && table->philo[table->i].meals >= table->max_meals)
			table->i++;
		if(table->i == table->number_of_philo)
			table->is_full = 0;
	}
}
...
int	start_simulation(t_table *table)
{
	...
	i = -1;
	table->first_eat_time = get_current_time();
	while(++i < table->number_of_philo)
	{
		if(pthread_create(&(table->philo + i)->philos, NULL, create_philo, table->philo + i))
			return (1);
